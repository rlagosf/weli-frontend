...
const tryGet = async (candidates = []) => {
  let lastStatus = 0;
  let lastMessage = "No se pudo obtener información";
  ...
  throw publicError(lastStatus, lastMessage);
};
 ...
 */
const normalizarConteoConvocatorias = (raw) => {
  const data = raw?.data ?? raw ?? {};
  ...
  return {
     ...
     */
    torneos_convocados: toNumberOrZero(
      data?.torneos_convocados ?? data?.convocatorias_total ?? data?.total_convocatorias
    ),
     ...
     */
    titular_partidos: toNumberOrZero(data?.titular_partidos ?? data?.titularidades ?? data?.veces_titular),
     ...
     */
    convocatorias_asistidas: toNumberOrZero(
      data?.convocatorias_asistidas ?? data?.asistencias ?? data?.total_asistencias
    ),

    convocatorias_ausentes: toNumberOrZero(
      data?.convocatorias_ausentes ?? data?.inasistencias ?? data?.total_inasistencias
    ),
  };
};
...
export const obtenerConteoConvocatorias = async (rut) => {
  ...
  const encodedRut = encodeURIComponent(rutStr);
  ...
  for (const path of candidates) {
    try {
      ...
      return normalizarConteoConvocatorias(data);
    } catch (error) {
      const status = Number(error?.status ?? 0);
...
export const obtenerJugador = async (rut) => {
  ...
  try {
     ...
     * jugador plano
     */
    const one = unwrapOne(direct);

    if (one) {
      const candidateRut = String(one?.rut_jugador ?? one?.rut ?? "").trim();
       ...
       * DTO reducido.
       */
      if (!candidateRut || candidateRut === rutStr) {
        return one;
      }
    }
     ...
     * retorna array.
     */
    const list = Array.isArray(direct?.items) ? direct.items : Array.isArray(direct) ? direct : [];

    const hit = list.find((jugador) => String(jugador?.rut_jugador ?? jugador?.rut ?? "").trim() === rutStr);
  ...
  try {
    const todos = await safeGet("/jugadores");

    const list = Array.isArray(todos?.items) ? todos.items : Array.isArray(todos) ? todos : [];

    const jugador = list.find((item) => String(item?.rut_jugador ?? item?.rut ?? "").trim() === rutStr);
 ...
 * }
 */
export const normalizarEstadisticasJoined = (raw) => {
  const root = raw?.data ?? raw ?? {};

  const item =
    root?.item && typeof root.item === "object"
      ? root.item
      : root?.data?.item && typeof root.data.item === "object"
        ? root.data.item
        : root;

  const base = item?.base && typeof item.base === "object" ? item.base : {};

  const sport = item?.sport && typeof item.sport === "object" ? item.sport : {};

  const isJoined = Object.keys(base).length > 0 || Object.keys(sport).length > 0;
 ...
 * stats_<deporte>
 */
export const obtenerEstadisticasJugador = async ({ jugadorId, deporteId } = {}) => {
  const jid = Number(jugadorId ?? 0);

  const did = Number(deporteId ?? 0);
  ...
  const encodedJugadorId = encodeURIComponent(String(jid));

  const deporteQuery = Number.isFinite(did) && did > 0 ? `?deporte_id=${encodeURIComponent(String(did))}` : "";
  ...
  try {
    ...
    return normalizarEstadisticasJoined(data);
  } catch (error) {
    /*
 ...
 * según deporte_id.
 */
export const obtenerContextoEstadisticoJugador = async (jugador) => {
  if (!jugador) {
    return {
      stats: generarStatsBaseVacias(),

      convocatorias: normalizarConteoConvocatorias({}),

      deporte_id: null,
    };
  }

  const jugadorId = Number(jugador?.id ?? jugador?.jugador_id ?? jugador?.id_jugador ?? 0);

  const deporteId = Number(jugador?.deporte_id ?? jugador?.id_deporte ?? 0);

  const rut = String(jugador?.rut_jugador ?? jugador?.rut ?? "").trim();
   ...
   * al jugador.
   */
  const [stats, convocatorias] = await Promise.all([
    obtenerEstadisticasJugador({
      jugadorId,
      deporteId,
    }),

    obtenerConteoConvocatorias(rut),
  ]);
  ...
  return {
    ...
    deporte_id: Number.isFinite(deporteId) && deporteId > 0 ? deporteId : null,
  };
};
