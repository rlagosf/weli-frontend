...
const extractTokenAcademiaId = (decoded) => {
  const raw =
    ...
    decoded?.scope?.academia_id ??
    decoded?.tenant?.academia_id ??
    decoded?.context?.academia_id ??
    0;

  const academiaId = Number(raw);

  return Number.isInteger(academiaId) && academiaId > 0 ? academiaId : null;
};
...
const extractTokenDeporteId = (decoded) => {
  const raw =
    decoded?.deporte_id ??
    decoded?.user?.deporte_id ??
    decoded?.scope?.deporte_id ??
    decoded?.tenant?.deporte_id ??
    decoded?.context?.deporte_id ??
    decoded?.sport_id ??
    decoded?.id_deporte ??
    0;
  ...
  return Number.isInteger(deporteId) && deporteId > 0 ? deporteId : null;
};
...
const buildAggPath = ({ academia_id, deporte_id }) => {
  ...
  if (academia_id) {
    qs.set("academia_id", String(Number(academia_id)));
  }

  if (deporte_id) {
    qs.set("deporte_id", String(Number(deporte_id)));
  }
...
export default function EstadisticasGlobales() {
  ...
  useEffect(() => {
    ...
    document.dispatchEvent(
      new CustomEvent("updateBreadcrumb", {
        detail: {
          title,
        },
      })
  ...
  useEffect(() => {
    ...
    if (isSuperTree && !scope.academia_id) {
      return;
    }
    ...
    (async () => {
      ...
      try {
        ...
        const [rawTodos, rawActivos] = await Promise.all([
          ...
          tryGetList(jugadoresActivosPaths, {
            signal: abort.signal,
          }),
        ]);
        ...
        const applyScopeFilter = (arr) => {
          ...
          const currentDeporteId = depId || scope.deporte_id;

          if (!academiaId && !currentDeporteId) {
            return safe;
          }

          return safe.filter((jugador) => {
            ...
            if (currentDeporteId && jugadorDeporteId !== currentDeporteId) {
              return false;
            }
        ...
        try {
          const response = await api.get(aggPath, {
            signal: abort.signal,
          });

          const aggRes = response?.data ?? null;
          ...
          const message =
            errorAggregate?.response?.data?.message ||
            errorAggregate?.response?.data?.detail ||
            errorAggregate?.message ||
            (status ? `Aggregate falló (${status})` : "Aggregate falló");
        ...
        setError(errorRequest?.response?.data?.message || errorRequest?.message || "Error al cargar datos");
      } finally {
        if (!abort.signal.aborted) {
          setIsLoading(false);
        }
  ...
  const conteos = useMemo(() => {
    ...
    const getCategoriaNombre = (jugador) =>
      jugador?.categoria?.nombre ??
      (jugador?.categoria_id != null ? catMap.get(Number(jugador.categoria_id)) : undefined);

    const getPosicionNombre = (jugador) =>
      jugador?.posicion?.nombre ?? (jugador?.posicion_id != null ? posMap.get(Number(jugador.posicion_id)) : undefined);

    const getEstadoNombre = (jugador) =>
      jugador?.estado?.nombre ?? (jugador?.estado_id != null ? estMap.get(Number(jugador.estado_id)) : undefined);

    const getSucursalNombre = (jugador) =>
      jugador?.sucursal?.nombre ?? (jugador?.sucursal_id != null ? sucMap.get(Number(jugador.sucursal_id)) : undefined);

    const getPrevisionNombre = (jugador) =>
      jugador?.prevision_medica?.nombre ??
      (jugador?.prevision_medica_id != null ? prevMap.get(Number(jugador.prevision_medica_id)) : undefined);
    ...
    activos.forEach((jugador) => {
      ...
      const key = Number.isFinite(edad) && edad >= 0 ? String(edad) : "—";
  ...
  const sumasPorGrupo = useMemo(() => {
    ...
    const sumGroup = (campos) => {
      ...
      for (const campo of campos) {
        const value = Number(totals?.[campo] ?? 0);

        result[campo] = Number.isFinite(value) ? value : 0;
      }
  ...
  const crearDatosBar = (datos) => {
    ...
    return {
      labels: keys.map((key) => traducciones[key] ?? key),
      ...
      datasets: [
        {
          ...
          borderColor: values.map((_, index) => hexToRgba(EVENT_COLORS[index % EVENT_COLORS.length], 0.9)),
  ...
  if (scope.deporte_id) {
    scopeLabelParts.push(`Deporte: ${sportMeta.nombre}`);
  }
      ...
      <main className="px-6 pb-20">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          {tarjetasPie.map(({ key, label, data }) => {
            const total = Object.values(data || {}).reduce((accum, value) => accum + (Number(value) || 0), 0);
          ...
          <div className="mt-8 max-w-6xl mx-auto">
            <div className={ui.card}>
              <p className={`text-center ${ui.headerSub}`}>
                ...
                .
                <br />
                Si existen estadísticas registradas, revisa la disponibilidad del endpoint de agregación y el deporte
                asociado al contexto actual.
              </p>
            </div>
          </div>
        )}
