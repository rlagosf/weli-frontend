...
/** Si baseURL incluye /api, pide "/academias". Si no, "/api/academias". */
function resolveAcademiasPath(baseURL) {
    const b = String(baseURL || "");
    const includesApi = /\/api\/?$/i.test(b) || /\/api\//i.test(b);
    return includesApi ? "/academias" : "/api/academias";
}

/** Tabla deportes: si baseURL incluye /api => "/deportes" else "/api/deportes" */
function resolveDeportesPath(baseURL) {
    const b = String(baseURL || "");
    const includesApi = /\/api\/?$/i.test(b) || /\/api\//i.test(b);
    return includesApi ? "/deportes" : "/api/deportes";
}
...
export default function SuperDashboard() {
    ...
    const academiasPath = resolveAcademiasPath(baseURL);
    const deportesPath = resolveDeportesPath(baseURL);
    ...
    const loadDeportes = useCallback(
        async (signal) => {
    ...
    const enterAcademia = (a) => {
        ...
        // ✅ Fuerza salto real (evita problemas de rutas anidadas / guards / basename)
        window.location.assign("/super-dashboard/admin/dashboard");
    };


    // Logout: igual que dashboard.jsx
