...
export default function PrevisionMedica() {
  ...
  const fetchDatos = useCallback(
    async (signal) => {
      try {
        const res = await apiOps.getVar("/prevision-medica", { signal });
        ...
        setError(prettyError(err, "❌ Error al obtener previsiones médicas"));
      }
    },
    [apiOps, handleAuth]
  );
  ...
  const crear = async () => {
    ...
    setBusy(true);
    try {
      await apiOps.postVar("/prevision-medica", { nombre });
      setNuevo("");
      flash("✅ Previsión médica creada");
      await fetchDatos();
    } catch (err) {
      const st = getErrStatus(err);
      if (st === 401 || st === 403) return handleAuth();
      setError(prettyError(err, "❌ No se pudo crear la previsión médica."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const actualizar = async () => {
    ...
    setBusy(true);
    try {
      await apiOps.putVar(`/prevision-medica/${editarId}`, { nombre });
      setEditarId(null);
      setEditarNombre("");
      flash("✅ Previsión médica actualizada");
      await fetchDatos();
    } catch (err) {
      const st = getErrStatus(err);
      if (st === 401 || st === 403) return handleAuth();
      setError(prettyError(err, "❌ No se pudo actualizar la previsión médica."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const eliminar = async () => {
    ...
    setBusy(true);
    try {
      await apiOps.delVar(`/prevision-medica/${seleccionado.id}`);
      flash("✅ Previsión médica eliminada");
      await fetchDatos();
    } catch (err) {
      const st = getErrStatus(err);
      if (st === 401 || st === 403) return handleAuth();
      setError(prettyError(err, "❌ No se pudo eliminar la previsión médica."));
    } finally {
      setBusy(false);
  ...
  return (
    <div className={`${ui.shell} min-h-screen font-sans`}>
      ...
      <header className="px-6 pt-6 text-center">
        <h1 className={`text-4xl font-extrabold tracking-tightish ${ui.titleMain}`}>Previsión Médica</h1>
        <p className={`text-sm mt-2 ${ui.subText}`}>Administra el catálogo de previsión (crear, editar, eliminar).</p>
      </header>
          ...
          <section className={`${ui.card} p-6 lg:col-span-2`}>
            <div className="flex items-baseline justify-between gap-3 mb-4">
              ...
              <span className={ui.pill}>
                {previsiones.length} registro{previsiones.length !== 1 ? "s" : ""}
              </span>
            </div>
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            <section className={`${ui.card} p-6`}>
              ...
              <button
                ...
                title={busy ? "Procesando..." : "Crear previsión"}
              >
            ...
            <section className={`${ui.card} p-6`}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setEditarId(id || null);
                  const sel = previsiones.find((x) => Number(x.id) === id);
              ...
              <button
                ...
                title={!editarId ? "Selecciona un registro primero" : busy ? "Procesando..." : "Actualizar"}
              >
            ...
            <section className={`${ui.card} p-6`}>
              ...
              <button
                ...
                title={!seleccionado ? "Selecciona un registro" : busy ? "Procesando..." : "Eliminar"}
              >
