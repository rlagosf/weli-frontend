...
export default function TiposPago() {
  ...
  // 🧭 Breadcrumb → lo pinta el layout (/admin). Aquí solo sincronizamos el estado.
  useEffect(() => {
    const currentPath = location.pathname;
    const bc = Array.isArray(location.state?.breadcrumb) ? location.state.breadcrumb : [];
    const last = bc[bc.length - 1];
    if (!last || last.label !== 'Tipos de Pago') {
      navigate(currentPath, {
        ...
        state: {
          ...
          breadcrumb: [
            ...
            { label: 'Tipos de Pago', to: currentPath },
          ],
        },
      });
    }
    ...
    <div className={`${fondo} min-h-screen px-4 pt-4 pb-16 font-realacademy`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-6xl mx-auto">
        ...
        <div className={`${tarjeta} border shadow-md rounded-xl p-6`}>
          <h3 className="text-lg font-bold mb-4">📋 Listar Tipos</h3>
          {tipos.length === 0 ? (
        ...
        {/* Crear */}
        <div className={`${tarjeta} border shadow-md rounded-xl p-6`}>
          <h3 className="text-lg font-bold mb-4">➕ Crear Tipo</h3>
          <input
            type="text"
            value={nuevo}
            onChange={(e) => {
        ...
        {/* Modificar */}
        <div className={`${tarjeta} border shadow-md rounded-xl p-6`}>
          <h3 className="text-lg font-bold mb-4">✏️ Modificar Tipo</h3>
          <select
            value={editarId || ''}
            onChange={(e) => {
        ...
        {/* Eliminar */}
        <div className={`${tarjeta} border shadow-md rounded-xl p-6`}>
          <h3 className="text-lg font-bold mb-4">🗑️ Eliminar Tipo</h3>
          <select
